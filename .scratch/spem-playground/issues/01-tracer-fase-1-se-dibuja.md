...
- [x] Vite + React + TypeScript scaffold runs with a dev server command
- [x] Vitest installed and a test command runs green (no jsdom, no testing-library)
- [x] Plain CSS design tokens defined as custom properties: paper `#f5f5f5`, ink
      `#2d3142`, accent `#eb6c36`, plus muted and rule hairline values
- [x] The `Modelo` / `Fase` / `Tarea` types exist as specified in the spec, and all four
      **Fases** from the document are seeded with their real **Roles**, **Tareas**,
      **Entrada** and **Salida** content
- [x] A pure layout function maps one **Fase** to absolute coordinates, measuring text by
      character-width approximation and never touching the DOM
- [x] A React component renders that layout as a single `<svg>` containing no geometry
      logic of its own — every number comes from the layout
- [x] **Fase 1** is visible in the browser and reads left-to-right: **Entrada** →
      **Tareas** → **Salida**
- [x] Arrows connect the **Entrada** panel to the **Tareas** column and the **Tareas**
      column to the **Salida** panel
- [x] Layout tests assert relationships, not pixel constants: **Entrada** panel entirely
...
- [x] Layout test: a **Tarea** whose text exceeds the node width wraps to multiple lines
